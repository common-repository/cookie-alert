...
Description: Show a cookie info on every page.
Version: 1.0
...
defined( 'ABSPATH' ) or die( 'Plugin file cannot be accessed directly motherfuckas!' );
	...
	class CookieAlert {
        ...
        public function cookiealert_stampa_messaggio() {
                   ...
                   $testo_bottone = (get_option('cookiealert_testobottone') == '' ? 'Chiudi' : get_option('cookiealert_testobottone'));
                   $colore_button = get_option('cookiealert_colorebottone');
                   $posizione = (get_option('cookiealert_posizione') == 'top' ? 'top:0;bottom:auto;' : 'bottom:0;top:auto;');
                   
             
            if($attivo == 'active') {
            
        echo '<div class="cookie-alert-wrap" style="'. $posizione .'display:none;">
        ...
        <a href="#" class="cookie-alert-chiudi" style="background-color:'. $colore_button .'">' .  $testo_bottone . '</a></div>';
